...
```
1. @manager-pj → PJ計画 
2. @manager-agent,@analyze-pj,@design-expert,@developer → PJ内部レビュー
3. @manager-pj → PJ計画改善
3. @design-expert → 詳細設計
4. @manager-agent,@analyze-pj,@design-expert,@developer → 詳細設計内部レビュー
3. @design-expert → 詳細設計改善
5. @design-test → テスト設計
6. @design-test,@developer → テスト設計内部レビュー
5. @design-test → テスト設計改善
7. @developer,@test-developer → 適切なブランチに切り替えTDDでの開発
...
- **開発する際は、まずブランチ状態を確認し、最適なブランチで作業を開始する**
- **成果物が完成したら、適切なブランチでghコマンドでPRを作成する**
- **ユーザーの停止指示まで自動継続**
- **ドキュメントは必ずdocs配下に作成**
